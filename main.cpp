...
color ray_color(ray &r, hittable_list &list, int depth) {
    ...
    if (list.hit(r, 0.001, inf, hrecord)) {
        ...
        return 0.75 * ray_color(newnorm, list, depth-1);
    }
...
Camera c = Camera(90.0, aspect_ratio, point3(0, 1, 0), point3(0, 0, 0), point3(0, 0, -1));
    ...
    for (int j = startheight; j >= endheight; --j) {
        ...
        for (int i = 0; i < image_width; ++i) {
            ...
            for (int k = 0; k < samples_per_pixel; k++) {
                ...
                color color_at_target = ray_color(r);
...
int main() {
    ...
    int numThreads = std::min(hardware_threads != 0 ? hardware_threads-2 : 2, max_threads);
    ...
    for (int j = numThreads - 1; j >= 0; j--) {
        ...
        for (auto x : outputVector[j]) {
            ...
            // std::cout << static_cast<int>(256 * clamp(r, 0, 0.999)) << " " << static_cast<int>(256 * clamp(r, 0, 0.999)) << " " << static_cast<int>(256 * clamp(r, 0, 0.999)) << '\n';
